...
<html lang="en">
<head>
    <link rel="stylesheet" href="buyDone.css">    
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Drill Machine</title>
</head>
<body>
    <!-- <div class="yay">
...
<?php    
    ...
    $email = $_POST['email'];
        ...
        $querySel ="SELECT qty FROM items WHERE name = 'BoschGBH2-26' ";
        ...
        if(mysqli_num_rows($queryResSel)>0){
            while($row  = mysqli_fetch_assoc($queryResSel)){
                ...
                if($q >= $quantity){
                    $newQuant = $q - $quantity;
                    $totAmt = $quantity*12350;
                    $book = "INSERT INTO orders VALUE('$quantity', '$totAmt' ,'BOSCH GBH 2-26 RE ROTARY HAMMER','$name','$address','$pin','$mobile','$email')";
                    $bookRes = mysqli_query($connect , $book);
                    $update = "UPDATE items  SET qty = '$newQuant' WHERE name='BoschGBH2-26'";
                    $updateRes = mysqli_query($connect , $update);
                    echo "<p> Qty : ".$quantity."</p>";
                    echo "<p>Total Amount: Rs.".$totAmt."</p>";
                    echo "<p> Product : BOSCH GBH 2-26 RE ROTARY HAMMER Drill</p>";
                    echo "<p> Name : ".$name."</p>";
